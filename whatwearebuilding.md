...
- 用戶零註冊（暱稱僅作識別；為保護個資，不提供公開自動帶入）
- 團購進度條（類 Kickstarter）驅動分享裂變
- Admin-only 登入，用戶端完全公開
- 完整訂單生命週期：下單 → 付款 → 確認 → 到貨通知 → 出貨
- **LINE 綁定推播**：用戶下單後將訂單編號 + 訂購人姓名 + 手機末三碼貼到 LINE 官方帳號，即可收到 1-on-1 推播通知（非廣播）
...
| 取貨方式 | 宅配 or 面交（統稱） | |
| 綁定訂單 | 用戶在 LINE OA 貼上訂單編號 + 訂購人姓名 + 手機末三碼，系統將 LINE user ID 寫入該訂單 | 用於 1-on-1 推播 |
...
送出訂單 → API Route（庫存檢查 + 運費計算 + submission_key 防重複）
      ↓
顯示銀行帳號 + 訂單編號 + 分享揪團 CTA
      ↓
用戶匯款 → 以訂單編號 + 訂購人姓名 + 手機末三碼回報匯款（金額 + 帳號後五碼）
      ↓
Admin 後台 → 確認付款（單筆 / 批次）→ LINE + Email 通知
      ↓
Admin 供應商管理 → 商品到貨 → 通知相關客戶「已到達理貨中心」
      ↓
Admin 待出貨 → 確認寄出（單筆 / 批次）→ LINE push + Email 出貨通知

用戶在 LINE OA 貼上訂單編號 + 訂購人姓名 + 手機末三碼 → Webhook 驗證 → 綁定 line_user_id → 後續通知走 1-on-1 push
```
...
| Notification Layer | 付款確認 + 到貨 + 出貨 + 取消通知，LINE 1-on-1 push/multicast + Email，失敗記錄可查 |
...
| Notifications | LINE Messaging API | 1-on-1 push/multicast，用戶綁定訂單後收個人化推播 |
...
5. 填寫資料頁：
   - LINE 暱稱（僅作識別與團務對照；不提供公開自動查詢/帶入）
   - 收貨人姓名、電話、地址、Email
   - 取貨方式（宅配 / 面交點 A / 面交點 B）
   - **若選宅配 → 顯示「宅配到以上地址，運費 $XXX」**（金額由 Admin 在開團時設定）
   - 訂單摘要（品項 + 小計 + 運費 + 合計）
   - 若暱稱已存在但收件資料不一致 → 公開流程拒絕覆寫，需改用其他暱稱或請團主協助
6. 點「送出訂單」→ 按鈕立即 disabled 防重複（client-side `submission_key`）
7. Server 驗證庫存 → 計算運費（宅配才加）→ 原子扣庫存 → 建立訂單 + 明細 → 回傳訂單編號

### Flow 2：匯款 + 回報

1. 訂單成立頁顯示：
   - 銀行帳號（銀行名、戶名、帳號、應付金額 — 含運費）
   - 訂單編號（之後查單、取消、回報匯款、LINE 綁定都需要搭配訂購人姓名 + 手機末三碼）
...
1. 用戶下單後，訂單確認頁顯示訂單編號（如 `ORD-20260318-001`），並提示「將訂單編號 + 訂購人姓名 + 手機末三碼一起貼到我們的 LINE 官方帳號，即可收到出貨通知」
2. 用戶開啟 LINE → 傳送 `ORD-20260318-001 王小美 678` 給官方帳號
3. LINE Webhook 收到訊息 → 驗證 signature → `handleMessage()` 辨識 `ORD-YYYYMMDD-NNN` + 訂購人姓名 + 手機末三碼
4. 系統查詢訂單 → 驗證訂單編號、訂購人姓名與手機末三碼匹配，且未被其他 LINE 帳號綁定 → 將 `line_user_id` 寫入該訂單
5. 回覆用戶：「訂單 ORD-20260318-001 已綁定成功 ✅ 出貨時我們會在這裡通知你！」
6. 之後 Admin 確認付款 / 出貨 / 取消時，系統自動透過 LINE push 發送 1-on-1 通知給該用戶
7. 若用戶已綁定過，再次傳送相同訂單編號 + 訂購人姓名 + 手機末三碼 → 顯示目前訂單狀態（冪等）
8. 若用戶只傳訂單編號或傳送非訂單格式的文字 → 回覆使用說明

**注意事項：**
- 每筆訂單只能綁定一個 LINE 帳號（per-order linking，非 per-user）
- 未綁定的訂單仍可透過 Email 收到通知，LINE push 會被跳過（不會報錯）
- 到貨通知（product_arrival）使用 multicast API，一次推播給所有綁定過的相關客戶
...
5. **到貨通知**：
   ...
   - 發送 LINE + Resend Email：「您訂購的【有機地瓜】已到達理貨中心，我們會盡快安排出貨！」
...
2. 開啟精簡版下單表單（同用戶端結帳流程，但嵌入 Admin 內）：
   - 選擇商品 + 數量（同用戶端 +/- 介面）
   - 填寫客戶暱稱（**Admin-only** 自動帶入舊客資料）+ 收貨人資訊 + 取貨方式
   - 顯示訂單摘要（含運費）
...
|--------|------|-------|
| id | UUID | Primary key |
| nickname | TEXT | UNIQUE，LINE 暱稱作為識別/團務對照，不是公開授權憑證 |
...
| line_user_id | TEXT | nullable，用戶透過 LINE OA 綁定後寫入，用於 1-on-1 push 通知 |
| created_at | TIMESTAMPTZ | |
...
| 觸發時機 | type | LINE 方式 | 內容 |
|----------|------|-----------|------|
| Admin 確認付款 | `payment_confirmed` | push（單一用戶） | 「訂單已確認，我們會盡快安排出貨」 |
| Admin 點「通知到貨」 | `product_arrival` | multicast（批次） | 「您訂購的【有機地瓜】已到達理貨中心」 |
| Admin 確認寄出 | `shipment` | push（單一用戶） | 「訂單已出貨 / 已備妥可取貨」 |
| Admin 取消訂單 | `order_cancelled` | push（單一用戶） | 「您的訂單已被取消。原因：{cancel_reason}」 |
...
4. **訂單查詢結果可點進詳情**：Lookup 頁結果可點擊跳轉到 `/order/[orderNumber]?code=...`，方便操作（回報匯款、取消等）
5. **訂單成立頁可返回商店**：顯示「繼續選購」按鈕回到商品列表（同一團繼續下單）
6. **單一開團**：系統同一時間只允許一個 open round（DB partial unique index 強制）。`?round=` 參數僅供明確深連結使用，非用於切換多團
...
5. Client 不論新建或重複，都導向同一訂單確認頁 `/order/{order_number}`

### 公開訂單驗證

1. `/lookup` 以 `recipient_name + phone_last3` 搜尋所有匹配訂單
2. 單筆訂單詳情、回報匯款、取消訂單、LINE 綁定，都以 `order_number + recipient_name + phone_last3` 驗證
3. 公開流程不可暴露內部 UUID
4. 公開 lookup / detail / report-payment / cancel-order 全部走 server-side API，不走 anon 直連資料表
...
| 風險 | 防護 |
|------|------|
| 匿名用戶直接讀取/修改核心個資與訂單 | `users` / `orders` / `order_items` 對 anon 全面封鎖，所有公開操作改走 server-side API |
| 匿名用戶讀取通知/供應商 | `notification_logs` + `suppliers` 僅 authenticated |
| 匿名用戶修改商品/團次 | `products` + `rounds` 寫入限 authenticated；只保留公開 SELECT |

### LINE Messaging API（1-on-1 Push）

用戶將訂單編號 + 訂購人姓名 + 手機末三碼貼到 LINE 官方帳號後，系統綁定 `line_user_id`，後續通知走 1-on-1 push（非 broadcast）。

**三種 API 端點：**

| 端點 | 用途 | 費用 |
|------|------|------|
| `POST /v2/bot/message/push` | 單一用戶推播（付款確認、出貨、取消） | 按訊息數計費 |
| `POST /v2/bot/message/multicast` | 批次推播（到貨通知，最多 500 人/次） | 按訊息數計費 |
| `POST /v2/bot/message/reply` | 回覆 Webhook 事件（綁定確認） | 免費 |

**Webhook 流程：**
```
LINE 用戶傳訊息 → POST /api/line/webhook
  → 驗證 x-line-signature (HMAC-SHA256)
  → 辨識訂單編號格式 (ORD-YYYYMMDD-NNN) + 訂購人姓名 + 手機末三碼
  → 驗證訂單 + 訂購人姓名 + 手機末三碼 + 綁定 line_user_id
  → Reply API 回覆確認訊息
```

**降級機制：** Reply token 過期時自動 fallback 到 push。未綁定 LINE 的訂單跳過 push，僅發 Email。

需要 `LINE_CHANNEL_ACCESS_TOKEN` 和 `LINE_CHANNEL_SECRET`。
...
| LINE Messaging API | 低量 MVP 的 push / multicast 成本可控 |
| **總計** | **$0/月** |
...
- [x] 用戶端：訂單查詢（以訂購人姓名 + 手機末三碼查詢，再以訂單編號 + 訂購人姓名 + 手機末三碼進入詳情頁操作）
...
- [x] 安全：鎖緊 RLS + server-side public identity checks + stock atomic + submission_key + advisory lock + 基本 rate limiting
- [x] 取貨方式 + 運費計算
