...
- **LINE 綁定推播**：用戶下單後將訂單編號貼到 LINE 官方帳號，即可收到 1-on-1 推播通知（非廣播）
...
| 取貨方式 | 宅配 or 面交（統稱） | |
| 綁定訂單 | 用戶在 LINE OA 貼上訂單編號，系統將 LINE user ID 寫入該訂單 | 用於 1-on-1 推播 |
...
用戶在 LINE OA 貼上訂單編號 → Webhook 驗證 → 綁定 line_user_id → 後續通知走 1-on-1 push
```
...
1. 用戶下單後，訂單確認頁顯示訂單編號（如 `ORD-20260318-001`）+ 提示「將訂單編號貼到我們的 LINE 官方帳號，即可收到出貨通知」
2. 用戶開啟 LINE → 傳送訂單編號給官方帳號
3. LINE Webhook 收到訊息 → 驗證 signature → `handleMessage()` 辨識格式為 `ORD-YYYYMMDD-NNN`
4. 系統查詢訂單 → 驗證未被其他 LINE 帳號綁定 → 將 `line_user_id` 寫入該訂單
5. 回覆用戶：「訂單 ORD-20260318-001 已綁定成功 ✅ 出貨時我們會在這裡通知你！」
6. 之後 Admin 確認付款 / 出貨 / 取消時，系統自動透過 LINE push 發送 1-on-1 通知給該用戶
7. 若用戶已綁定過，再次傳送相同訂單編號 → 顯示目前訂單狀態（冪等）
8. 若用戶傳送非訂單編號的文字 → 回覆使用說明
...
用戶將訂單編號貼到 LINE 官方帳號後，系統綁定 `line_user_id`，後續通知走 1-on-1 push（非 broadcast）。
...
LINE 用戶傳訊息 → POST /api/line/webhook
  ...
  → 辨識訂單編號格式 (ORD-YYYYMMDD-NNN)
  → 驗證訂單 + 綁定 line_user_id
  → Reply API 回覆確認訊息
```
...
| LINE Messaging API | Free tier（broadcast 免費額度足夠） |
| **總計** | **$0/月** |
...
- [ ] Admin：待出貨管理（按取貨方式分組 + 單筆/批次確認寄出 + 出貨通知）
- [x] Admin：商品管理（CRUD + 庫存 + 目標 + 圖片 + 供應商關聯）
- [x] Admin：開團管理（開團/截單/截止時間/運費設定）
- [ ] Admin：供應商管理（CRUD + 依商品分組檢視客戶 + 到貨通知）
