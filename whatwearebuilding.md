...
| Notification Layer | 付款確認 + 到貨 + 出貨 + 取消通知，LINE Messaging API + Email，失敗記錄可查 |
...
| Notifications | LINE Messaging API | Broadcast API，推播通知到 LINE 群組 |
...
使用 Broadcast API 推播通知到所有 bot 好友。需要 `LINE_CHANNEL_ACCESS_TOKEN` 和 `LINE_CHANNEL_SECRET`。
