...
- **LINE 綁定推播**：用戶下單後將訂單編號 + 查詢碼貼到 LINE 官方帳號，即可收到 1-on-1 推播通知（非廣播）
...
| 取貨方式 | 宅配 or 面交（統稱） | |
| 綁定訂單 | 用戶在 LINE OA 貼上訂單編號 + 查詢碼，系統將 LINE user ID 寫入該訂單 | 用於 1-on-1 推播 |
...
用戶點連結 → Next.js 頁面（商品列表 + 進度條 + 購物車）
      ↓
送出訂單 → API Route（庫存檢查 + 運費計算 + submission_key 防重複 + access_code 產生）
      ↓
顯示銀行帳號 + 訂單編號 + 查詢碼 + 分享揪團 CTA
      ↓
用戶匯款 → 以訂單編號 + 查詢碼回報匯款（金額 + 帳號後五碼）
      ↓
...
用戶在 LINE OA 貼上訂單編號 + 查詢碼 → Webhook 驗證 → 綁定 line_user_id → 後續通知走 1-on-1 push
```
...
7. Server 驗證庫存 → 計算運費（宅配才加）→ 原子扣庫存 → 建立訂單 + 明細 → 回傳訂單編號 + 12 碼查詢碼

### Flow 2：匯款 + 回報

1. 訂單成立頁顯示：
   - 銀行帳號（銀行名、戶名、帳號、應付金額 — 含運費）
   - 訂單編號 + **12 碼查詢碼**（之後查單、取消、回報匯款、LINE 綁定都需要）
...
1. 用戶下單後，訂單確認頁顯示訂單編號（如 `ORD-20260318-001`）+ 12 碼查詢碼，並提示「將兩者一起貼到我們的 LINE 官方帳號，即可收到出貨通知」
2. 用戶開啟 LINE → 傳送 `ORD-20260318-001 ABCD1234EFGH` 給官方帳號
3. LINE Webhook 收到訊息 → 驗證 signature → `handleMessage()` 辨識 `ORD-YYYYMMDD-NNN` + 12 碼查詢碼
4. 系統查詢訂單 → 驗證訂單編號與查詢碼匹配，且未被其他 LINE 帳號綁定 → 將 `line_user_id` 寫入該訂單
5. 回覆用戶：「訂單 ORD-20260318-001 已綁定成功 ✅ 出貨時我們會在這裡通知你！」
6. 之後 Admin 確認付款 / 出貨 / 取消時，系統自動透過 LINE push 發送 1-on-1 通知給該用戶
7. 若用戶已綁定過，再次傳送相同訂單編號 + 查詢碼 → 顯示目前訂單狀態（冪等）
...
| access_code | TEXT | UNIQUE, 12 碼查詢碼，公開查單/回報匯款/取消/LINE 綁定都需要 |
...
5. Client 不論新建或重複，都導向同一訂單確認頁 `/order/{order_number}?code={access_code}`

### 查詢碼（`access_code`）

1. Server 在建立訂單時產生 12 碼英數查詢碼
2. 查詢碼與訂單編號成對使用，作為公開訂單操作的 capability
3. 需要查詢碼的公開流程：
   - `/lookup`
   - 訂單詳情頁
   - 回報匯款
   - 取消訂單
   - LINE 綁定
4. 不可再用暱稱、電話後四碼、或內部 UUID 當作公開授權憑證
...
用戶將訂單編號 + 查詢碼貼到 LINE 官方帳號後，系統綁定 `line_user_id`，後續通知走 1-on-1 push（非 broadcast）。
...
LINE 用戶傳訊息 → POST /api/line/webhook
  ...
  → 辨識訂單編號格式 (ORD-YYYYMMDD-NNN) + 12 碼查詢碼
  → 驗證訂單 + 查詢碼 + 綁定 line_user_id
  → Reply API 回覆確認訊息
```
...
- [x] 安全：鎖緊 RLS + access_code + stock atomic + submission_key + advisory lock + 基本 rate limiting
- [x] 取貨方式 + 運費計算
